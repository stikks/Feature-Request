...
def ajax_feature_requests_list(client_slug):
    ...
    data = feature_schema.dump(feature.FeatureService.objects_filter(**dict(client_id=client_obj.id)), many=True).data

    print(data)

    return jsonify({
        'aaData': [[c.get('title'), c.get('description'), c.get('priority'), c.get('target_date'), ''] for c in data]
    }), 200
